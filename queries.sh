...
echo "$($PSQL "SELECT name from teams full join games on teams.team_id = games.winner_id or teams.team_id = games.opponent_id where games.round like 'Eighth-Final' and year=2014 group by name")"
...
echo "$($PSQL "SELECT name from teams full join games on teams.team_id = games.winner_id and teams.team_id = games.opponent_id where name like 'Co%'")"
